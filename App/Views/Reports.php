...
<script>
    ...
    /**
 * Pure JavaScript implementation of charts for inventory management system
 * This file replaces Chart.js dependency with custom canvas drawing
 */
...
function drawSalesTrendChart() {
    ...
    const dates = JSON.parse('<?= json_encode(array_column($dailySalesData, "date")) ?>');
    const salesData = JSON.parse('<?= json_encode(array_column($dailySalesData, "sales")) ?>');
...
function drawSalesByCategoryChart() {
    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        ctx.fillText(`LKR ${revenues[i].toLocaleString()} (${percentage}%)`, x + 20, y + 17);
    }
}
